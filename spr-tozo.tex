...
\subsection*{1a Nie było w sisper solutions) $L = \{w \in \{0, 1\}^* \mid w \text{ kończy się jedynką}\}$}
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{Zadanie1.jpeg}
    \caption{Zadanie 1}
...
\newpage
\section{Zadanie 2 - na zajęciach}
...
\section{Zadanie 1 - nie znalazlam w sipserze, robione na zajęciach}
...
\section{Zadanie 2 Nie było w sipserze - robione na zajęciach}
...
\section{Zadanie 3 - z sipser solutions}

Niech $s_1, s_2, s_3, ...$ będzie listą wszystkich słów w $\Sigma$*. Poniższa Maszyna Turinga rozpoznaje język $\overline{E_{TM}}$
...
\section{Zadanie 4 - z sipser solutions}
...
\section{Zadanie 5 z sipser solutions}

Poniższa TM rozstrzyga język $K$. \\
\textit{,,Na wejściu $\langle M \rangle$, gdzie M jest TM:}
\begin{enumerate}
    \item Skonstruuj DFA $A$, który akceptuje każdy ciąg zawierający nieparzystą liczbę jedynek.
    \item Skonstruuj DFA $B$ taki, że $L(B) = L(M) \cap L(A)$.
    \item Uruchom decyder T dla języka $E_{\text{DFA}}$ na wejściu $\langle B \rangle$.
    \item Jeśli $T$ akceptuje, zaakceptuj; jeśli $T$ odrzuca, odrzuć.''
\end{enumerate}



\section{Zadanie 6 - robiliśmy na zajęciach}
...
\section{Zadanie 7 - robiliśmy na zajęciach}
\subsection{Suma $ L_1 \cup L_2$}
...
\setcounter{section}{0}
\section{Zadanie 1 - tego nie było, rozw ok}

Aby wykazać, że $EQ_{CFG}$ jest nierozstrzygalny, możemy pokazać, że $ALL_{CFG} \le_M EQ_{CFG}$. 

Poniższa Maszyna Turinga $M$ oblicza tę redukcję:

$M = $ „Dla wejścia $\langle G \rangle$, gdzie $G$ jest gramatyką bezkontekstową (CFG):
\begin{enumerate}
...
\section{Zadanie 2 - tylko a) z sipser solutions, rozw ok}
...
\section{Zadanie 3 - w sipserze, rozw ok}
...
\section{Zadanie 5 - w sipserze podobne, ale inne}
...
\section{Zadanie 1 - w sipserze, rozw ok}
...
\section{Zadanie 2 - na zajeciach robiliśmy, rozw ok}
...
\section{Zadanie 4 - na zajęciach}
Jak PATH $\in$ NPC, to P = NP 
\begin{enumerate}
    \item PATH $\in$ NPC \{zał.\}
    \item P $\subseteq$ NP \{wcześniej udowodnione twierdzenie\}\\
    Mamy udowodnić, że NP $\subseteq$ P. W tym celu:
    \item Niech A $\in$ NP \{zał. dodatkowe\}
    \item A $\leq_p$ PATH \{1, 3\}
    \item PATH $\in$ P \{wcześniej udowodnione twierdzenie\}
    \item A $\in$ P \{4, 5\}
\end{enumerate}

\section{Zadanie 5 w Sipserze, rozw ok}
\begin{enumerate}
...
\section{Zadanie 6 w Sipserze, rozw ok}
